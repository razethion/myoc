...
const config = {
    ...
    targetR2: optionValue('--target-r2') || envValue('TARGET_R2_BUCKET') || 'myoc',
    approvalSeed: optionValue('--approval-seed') || envValue('DEVELOPMENT_APPROVAL_SEED') || randomUUID(),
    concurrency: normalizeInteger(optionValue('--concurrency') || envValue('SEED_R2_CONCURRENCY') || '8', 1, 16),
    workerPort: normalizeInteger(envValue('SEED_R2_WORKER_PORT') || '8798', 1024, 65535),
...
async function replaceTargetR2(keys) {
    ...
    const workerFile = resolve(tempDir, 'seed-r2-worker.mjs')
    const workerConfig = resolve(tempDir, 'wrangler.seed-r2.jsonc')
    await writeFile(workerFile, await readFile(workerSourceFile))
    await writeFile(
        workerConfig,
        JSON.stringify(
            {
                name: 'myoc-development-seed',
                main: './seed-r2-worker.mjs',
                compatibility_date: '2026-09-06',
                r2_buckets: [
                    {binding: 'PRODUCTION_MEDIA_BUCKET', bucket_name: config.sourceR2, remote: true},
                    {binding: 'DEVELOPMENT_MEDIA_BUCKET', bucket_name: config.targetR2, remote},
                ],
            },
            null,
            2,
    ...
    try {
        const response = await fetch(`http://127.0.0.1:${config.workerPort}/seed`, {
            method: 'POST',
            headers: {'content-type': 'application/json'},
            body: JSON.stringify({keys, concurrency: config.concurrency}),
        })
        if (!response.ok || !response.body) throw new Error(`The R2 seed worker failed with status ${response.status}.`)
        const summary = await readWorkerEvents(response)
        console.log(`R2 seed complete: ${summary.cleared} cleared and ${summary.copied} copied.`)
    } finally {
...
async function startWorker(workerConfig) {
    ...
    const state = {child, exited: false, logs: ''}
    ...
    for (let attempt = 0; attempt < 120; attempt += 1) {
        if (state.exited) throw new Error(`The R2 seed worker stopped before it was ready.\n${state.logs}`)
        try {
            const response = await fetch(`http://127.0.0.1:${config.workerPort}/health`)
    ...
    throw new Error(`The R2 seed worker did not start.\n${state.logs}`)
}
...
async function readWorkerEvents(response) {
    ...
    for (;;) {
        ...
        for (const event of events) {
            const result = consumeWorkerEvent(event)
            if (result) summary = result
        }
        if (done) break
    }
    if (!summary) throw new Error('The R2 seed worker did not return a summary.')
    return summary
}

function consumeWorkerEvent(event) {
    const eventName = event.match(/^event: (.+)$/m)?.[1]
    const dataText = event.match(/^data: (.+)$/m)?.[1]
    if (!eventName || !dataText) return undefined
    const data = JSON.parse(dataText)
    if (eventName === 'progress') console.log(data.message)
    if (eventName === 'error') throw new Error(data.error)
    return eventName === 'summary' ? data : undefined
}
