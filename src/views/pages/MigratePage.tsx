...
(function () {
    ...
    async function fetchToyhouseImage(url) {
        ...
        if (blob.size > galleryImageMaxBytes) throw new Error('Toyhou.se image was larger than 200 MB: ' + url);
        let bitmap;
        try {
            bitmap = await createImageBitmap(blob, { colorSpaceConversion: 'default' });
            if (bitmap.width * bitmap.height > galleryImageMaxPixels) {
                throw new Error('Toyhou.se image was larger than 200,000,000 pixels: ' + url);
            }
            return {
                blob,
                contentType
