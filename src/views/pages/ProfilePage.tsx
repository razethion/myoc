...
function FolderIcon() {
    return (
        <svg aria-hidden="true" class="relative z-10 h-full w-full" fill="none" viewBox="0 0 200 200"
             xmlns="http://www.w3.org/2000/svg">
            <path d="M18 54c0-12 10-22 22-22h38c8 0 15 4 19 11l8 13h55c12 0 22 10 22 22v76c0 12-10 22-22 22H40c-12 0-22-10-22-22V54Z"
                  opacity="0.35" stroke="#000000" stroke-linejoin="round" stroke-width="12"
                  transform="translate(3 4)"/>
            <path d="M18 54c0-12 10-22 22-22h38c8 0 15 4 19 11l8 13h55c12 0 22 10 22 22v76c0 12-10 22-22 22H40c-12 0-22-10-22-22V54Z"
                  opacity="0.72" stroke="#ffffff" stroke-linejoin="round" stroke-width="2"/>
...
function SocialIcon({platform}: { platform: SocialPlatform }) {
    ...
    if (platform === 'furaffinity') {
        return (
            <svg aria-hidden="true" class="w-8" fill="currentColor" viewBox="0 0 32 32"
                 xmlns="http://www.w3.org/2000/svg">
    ...
    if (platform === 'bluesky') {
        return (
            <svg aria-hidden="true" class="h-6 w-6" fill="currentColor" viewBox="0 0 600 530"
                 xmlns="http://www.w3.org/2000/svg">
                        ...
                        {visibleSocialLinks.map((link) => {
                            ...
                            return (
                                <a aria-label={label}
                                   ...
                                   href={link.url} rel="noopener noreferrer" target="_blank" title={label}>
                ...
                {isFolderPage ? (
                    <nav aria-label="Folder breadcrumbs" class="breadcrumbs mb-5 justify-center overflow-x-auto text-sm">
                        <ul class="justify-center">
                            <li>
                                <a class="rounded border border-base-300 bg-base-200 px-2 py-1 font-semibold leading-none text-base-content hover:bg-base-300"
                                   href={profileUrl(profileUser.username)}>{profileUser.username}</a>
                            </li>
                            {folderPath.map((folder, index) => {
                                ...
                                return (
                                    <li>
                                        {isCurrentFolder ? (
                                            <span class="rounded border border-primary bg-primary px-2 py-1 font-bold leading-none text-primary-content">{folder.name}</span>
                                        ) : (
                                            <a class="rounded border border-base-300 bg-base-200 px-2 py-1 font-semibold leading-none text-base-content hover:bg-base-300"
                                               href={href}>{folder.name}</a>
                                        )}
                                    </li>
                                )
                            })}
                ...
                {childFolders.length > 0 ? (
                    <section aria-label="Folders" class="grid grid-cols-3 gap-3 sm:gap-6 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                        {childFolders.map((folder) => (
                            <a aria-label={`View folder ${folder.name}`} class="group block"
